// Web Worker for curve baking - offloads expensive bezier calculations from main thread
...
// Bake all 4 curves into a combined RGBA Float32Array
const bakeCombinedCurves = (sizeCurve, opacityCurve, velocityCurve, rotationSpeedCurve) => {
  ...
  for (let i = 0; i < CURVE_RESOLUTION; i++) {
    rgba[i * 4] = sizeData[i]; // R - size
    rgba[i * 4 + 1] = opacityData[i]; // G - opacity
    rgba[i * 4 + 2] = velocityData[i]; // B - velocity
    rgba[i * 4 + 3] = rotationSpeedData[i]; // A - rotation speed
  }
