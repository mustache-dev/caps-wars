...
const toRotation3D = (value) => {
  ...
  if (Array.isArray(value)) {
    ...
    if (Array.isArray(value[0])) {
      return [toRange(value[0], [0, 0]), toRange(value[1], [0, 0]), toRange(value[2], [0, 0])]
    }
...
let curveWorker = null
let curveWorkerCallbacks = new Map()
let curveWorkerIdCounter = 0

const getCurveWorker = () => {
  if (!curveWorker) {
...
const bakeCurvesAsync = (sizeCurve, opacityCurve, velocityCurve, rotationSpeedCurve) => {
...
// Create a default linear texture (1→0 fade) for immediate use while worker computes
const createDefaultCurveTexture = () => {
  const rgba = new Float32Array(CURVE_RESOLUTION * 4)
  for (let i = 0; i < CURVE_RESOLUTION; i++) {
...
const useCurveTextureAsync = (
  ...
  useEffect(() => {
    ...
    if (curveTexturePath) {
      ...
      fetch(curveTexturePath)
        .then((response) => {
          if (!response.ok) throw new Error(`HTTP ${response.status}`)
          return response.arrayBuffer()
        })
        .then((buffer) => {
          if (pendingRef.current !== requestId || !textureRef.current) return
        ...
        .catch((err) => {
          ...
          bakeCurvesAsync(sizeCurve, opacityCurve, velocityCurve, rotationSpeedCurve).then(
            (rgba) => {
      ...
      // Bake curves in worker (flexible path)
...
export const VFXParticles = forwardRef(function VFXParticles(
  {
    ...
    fadeSize = [1, 1], // ADDITIVE: No fade by default (start=end)
    depthTest = true,
    ...
    fadeOpacity = [1, 1], // ADDITIVE: No fade by default (start=end)
    ...
    gravity = [0, 0, 0],
    lifetime = [1, 2],
    // ADDITIVE: Spread in all directions by default
    direction = [
      [-1, 1],
      [-1, 1],
      [-1, 1],
    ...
    speed = [0.1, 2], // ADDITIVE: Varied speed by default
    friction = null, // ADDITIVE: Disabled by default (was { intensity: 0, easing: 'linear' })
    ...
    curveTexturePath = null, // Path to pre-baked curve texture (skips runtime baking for faster load)
  ...
  // ADDITIVE SYSTEM: Compute which features are active (non-default)
  // This allows conditional uniform creation and shader computation
  const activeFeatures = useMemo(
    () => ({
      // Direction is always needed for particle movement, but check if it has variation
      hasDirectionVariation: direction3D.some(([min, max]) => min !== max),
      // Start position offset
      hasStartPositionOffset: startPosition3D.some(([min, max]) => min !== 0 || max !== 0),
      // Rotation
      hasRotation: rotation3D.some(([min, max]) => min !== 0 || max !== 0),
      hasRotationSpeed: rotationSpeed3D.some(([min, max]) => min !== 0 || max !== 0),
      // Physics
      hasGravity: gravity.some((v) => v !== 0),
      hasTurbulence: turbulence?.intensity > 0,
      hasAttractors: attractors?.length > 0,
      hasFriction: friction !== null && frictionIntensityRange.some((v) => v !== 0),
      // Colors
      hasColorTransition: colorEnd !== null,
      hasMultipleStartColors: colorStart.length > 1,
      // Simple color: single start color, no transition - no per-particle storage needed
      needsPerParticleColor: colorStart.length > 1 || colorEnd !== null,
      // Curves
      hasFadeSizeCurve: fadeSizeCurve !== null,
      hasFadeOpacityCurve: fadeOpacityCurve !== null,
      hasVelocityCurve: velocityCurve !== null,
      hasRotationSpeedCurve: rotationSpeedCurve !== null,
      hasAnyCurve:
        fadeSizeCurve !== null ||
        ...
        curveTexturePath !== null,
      // Fading (check if values actually fade, i.e. start !== end)
      hasSizeFade: fadeSizeRange[0] !== fadeSizeRange[1],
      hasOpacityFade: fadeOpacityRange[0] !== fadeOpacityRange[1],
      // Other features
      hasCollision: collision !== null,
      hasSoftParticles: softParticles,
      hasAttractToCenter: attractToCenter,
      hasStartPositionAsDirection: startPositionAsDirection,
      // Emitter shape (BOX is default, others need special handling)
      hasNonBoxEmitter: emitterShape !== EmitterShape.BOX,
    }),
    [
      direction3D,
      startPosition3D,
  ...
  const {
    ...
    return arrays
  }, [activeMaxParticles, activeFeatures.needsPerParticleColor])
              ...
              .select(
                ...
                idx
                  ...
                  .select(
                    ...
                    idx
                      .lessThan(5)
                      .select(c4, idx.lessThan(6).select(c5, idx.lessThan(7).select(c6, c7)))
                  )
              )
          )
      )
  ...
  const computeInit = useMemo(() => {
    return Fn(() => {
      ...
      // ADDITIVE: Only initialize color arrays if they exist
      if (activeFeatures.needsPerParticleColor) {
  ...
  const computeSpawn = useMemo(() => {
    return Fn(() => {
      ...
      If(inRange, () => {
        ...
        // ADDITIVE: Core hashes - always needed
        ...
        // ADDITIVE: Rotation hashes - only if rotation is used
        const randRotationX = activeFeatures.hasRotation ? hash(particleSeed.add(2222)) : float(0)
        const randRotationY = activeFeatures.hasRotation ? hash(particleSeed.add(3333)) : float(0)
        const randRotationZ = activeFeatures.hasRotation ? hash(particleSeed.add(4444)) : float(0)

        // ADDITIVE: Start position hashes - only if offset is used
        const randPosX = activeFeatures.hasStartPositionOffset
          ? hash(particleSeed.add(5555))
          : float(0.5)
        const randPosY = activeFeatures.hasStartPositionOffset
          ? hash(particleSeed.add(6666))
          : float(0.5)
        const randPosZ = activeFeatures.hasStartPositionOffset
          ? hash(particleSeed.add(7777))
          : float(0.5)

        // ADDITIVE: Color end hash - only if color transition is used
        const randColorEnd = activeFeatures.hasColorTransition
          ? hash(particleSeed.add(888))
          : randColorStart

        // ADDITIVE: Emitter shape hashes - only if non-BOX emitter
        const randRadius = activeFeatures.hasNonBoxEmitter ? hash(particleSeed.add(8880)) : float(0)
        const randTheta = activeFeatures.hasNonBoxEmitter ? hash(particleSeed.add(9990)) : float(0)
        const randPhi = activeFeatures.hasNonBoxEmitter ? hash(particleSeed.add(10100)) : float(0)
        const randHeight = activeFeatures.hasNonBoxEmitter
          ? hash(particleSeed.add(11110))
          : float(0)
        ...
        // ADDITIVE: Only assign per-particle colors if arrays exist
        if (activeFeatures.needsPerParticleColor) {
  ...
  const computeUpdate = useMemo(() => {
    return Fn(() => {
      ...
      If(lifetime.greaterThan(0), () => {
        // ADDITIVE: Only compute gravity if enabled
        if (activeFeatures.hasGravity) {
        ...
        // ADDITIVE: Curl noise turbulence - only compile if enabled
        if (activeFeatures.hasTurbulence) {
          ...
          // Only apply if turbulence intensity > 0 (runtime check for debug panel changes)
        ...
        // ADDITIVE: Attractors - only compile if any attractors defined
        if (activeFeatures.hasAttractors) {
        ...
        // ADDITIVE: Plane collision - only compile if enabled
        if (activeFeatures.hasCollision) {
        ...
        // ADDITIVE: Rotation speed - only compile if has rotation speed
        if (activeFeatures.hasRotationSpeed) {
  ...
  const material = useMemo(() => {
    ...
    // ADDITIVE: Use per-particle color arrays only if they exist, otherwise use uniform directly
    let currentColor
    let pColorStart, pColorEnd
    if (activeFeatures.needsPerParticleColor) {
      pColorStart = particleColorStarts.element(instanceIndex)
      pColorEnd = particleColorEnds.element(instanceIndex)
      currentColor = mix(pColorStart, pColorEnd, progress)
    } else {
      // Simple color mode: just use the single uniform color
      pColorStart = uniforms.colorStart0
      pColorEnd = uniforms.colorStart0
      currentColor = uniforms.colorStart0
    }
    const intensifiedColor = currentColor.mul(uniforms.intensity)

    // ADDITIVE: Only sample curve texture if any curve is active
    // This saves a texture fetch per fragment when no curves are used
    let curveSample = null
    if (activeFeatures.hasAnyCurve) {
      ...
      curveSample = texture(curveTexture, vec2(progress, float(0.5)))
    }

    // Size multiplier: use curve if enabled and sampled, otherwise interpolate fadeSize prop
    const sizeMultiplier = activeFeatures.hasAnyCurve
      ? uniforms.fadeSizeCurveEnabled.greaterThan(0.5).select(
    ...
    // Opacity multiplier: use curve if enabled and sampled, otherwise interpolate fadeOpacity prop
    const opacityMultiplier = activeFeatures.hasAnyCurve
      ? uniforms.fadeOpacityCurveEnabled.greaterThan(0.5).select(
    ...
    if (activeGeometry) {
      ...
      const effectiveVelocityMultiplier = activeFeatures.hasAnyCurve
        ? uniforms.velocityCurveEnabled.greaterThan(0.5).select(curveSample.z, float(1))
        : float(1)
    ...
    (newValues) => {
      ...
      if ('friction' in newValues) {
        if (newValues.friction) {
          ...
          // Friction disabled (null) - reset to zero
          uniforms.frictionIntensityStart.value = 0
          uniforms.frictionIntensityEnd.value = 0
          uniforms.frictionEasingType.value = 0
        }
      }
      ...
      switch (name) {
        ...
        case 'DodecahedronGeometry':
          return {
            ...
            geometryArgs: { radius: params.radius ?? 0.5, detail: params.detail ?? 0 },
          }
